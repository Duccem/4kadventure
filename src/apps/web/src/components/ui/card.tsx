...
export default async function Card({ imageUrl, description, index }: CardProps) {

    return (
        <Link href={{
            pathname: `/destination/${index}/destinationInfo`,
        }}>
            <div>
                        ...
                        <CarouselContent>
                            <CarouselItem key={index}>
                                <div className="transition-transform transform hover:scale-110">
                                    <img src={imageUrl} alt="Card Image" className="w-full h-[450px] object-cover rounded-lg" style={{ imageRendering: 'pixelated' }} />
                                </div>
