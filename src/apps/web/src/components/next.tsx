...
const Next = async ({ tours }: { tours: any[] }) => {
  ...
  return (
    <div className="min-h-screen py-10 px-8 md:px-16 space-y-8">
      <p className="text-4xl md:text-6xl font-medium md:w-1/3">
        {t("next.title")}
      </p>
      <p className="text-2xl font-light md:w-1/3">{t("next.subtitle")}</p>
      <div className="">
              ...
              <CarouselItem key={index}>
                <div className="flex flex-col md:flex-row items-start justify-between gap-3">
                  <div className="md:w-1/2 rounded-xl overflow-hidden">
                    <img
                      src={`${tour.Fotos[0].url}`}
                      alt=""
                      className="object-cover"
                    />
                  </div>
                  <div className="md:w-1/2 flex flex-col gap-4 md:px-8">
                    <p className="text-4xl font-medium">{tour.Destination}</p>
                    <RichText content={tour.Description} />
                    <div className="flex items-center gap-4">
                      <Calendar className="size-6" />
                      <p className="text-xl">{tour.Date}</p>
                    </div>
                    <p className="text-2xl font-medium">${tour.Price}</p>
                    <div className="flex items-center gap-4">
                      <Users className="size-6" />
                      <p className="text-xl">{tour.Spots} spots available</p>
                    </div>
                    <div className="w-fit">
                      <Link
                        href={"/destination"}
